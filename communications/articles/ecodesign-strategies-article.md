...
> **Status:** skeleton draft. Full prose to be written after the tutorial-and-documentation specs ship; the SSOT vocabulary and updated mkdocs structure will inform the final wording. The umbrella mkdocs page `why_efootprint.md` is already published and frames the three pillars at navigator depth; this article expands on it for the Boavizta channel and links into its anchors.
...
- **Companion mkdocs page:** `docs_sources/mkdocs_sourcefiles/why_efootprint.md` (already published — umbrella the article expands on)
...
None of the three strategies replaces the others. The recap table comparing what each gives you and what each cannot give alone lives in the umbrella mkdocs page at `why_efootprint.md#the-three-together` — the article reuses it by reference rather than duplicating it, to avoid drift.
...
- `why_efootprint.md#best-practices-the-catalogue-of-possible-moves` — cited in §2 (replaces the former standalone `best_practices.md`)
- `why_efootprint.md#measurement-the-snapshot-of-what-is` — linked from §3 (replaces the former planned standalone `measurement_tools.md`)
- `why_efootprint.md#modeling-the-projection-that-lets-you-decide` — natural §4 anchor for the umbrella's framing of the simulation pillar
- `why_efootprint.md#the-three-together` — natural §5 anchor for the recap table
...
- **Headline hook (Title D):** *"Best practices tell you what's possible. Measurement tells you what is. Only simulation tells you what to prioritize."* — high-impact opener, derives directly from the umbrella's recap table (`why_efootprint.md#the-three-together`). Use as a standalone LinkedIn post around article launch.
...
- *"Compression, lazy-loading, efficient regions — useful, but which one matters for **your** service?"* — derived from `why_efootprint.md#best-practices-the-catalogue-of-possible-moves`.
- *"Measurement calibrates the model. Simulation turns it into a decision."* — restates the cooperative stance from `why_efootprint.md#the-three-together`.
- *"Why a PM can read an e-footprint model: usage journeys, not raw request counts."* — derived from `why_efootprint.md#modeling-the-projection-that-lets-you-decide`; doubles as cross-promotion for Article #3.
- *"Leaving the parts you can't measure out of scope is a hypothesis: that they're worth zero. Modeling makes the alternative explicit."* — derived from `why_efootprint.md#measurement-the-snapshot-of-what-is`. Reframes the measurement-only stance as a choice, not a default.
- *"Best practices and measurement are catalogues and snapshots. Modeling is the decision arrow that connects them."* — derived from `why_efootprint.md#the-three-together`. One-line landing of the cooperative frame.
