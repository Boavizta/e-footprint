...
class RecurrentEdgeComponentNeed(ModelingObject):
    """A repeating week-long resource demand placed on one {class:EdgeComponent} (RAM, CPU, storage, or whole-device workload). Each {class:EdgeUsagePattern} replays it for that pattern's deployment calendar and shared usage span."""
    ...
    def unitary_hourly_need_per_usage_pattern(self, usage_pattern: "EdgeUsagePattern"):
        """Hourly resource demand for one edge device, generated by replaying the typical week over the deployment calendar and scaled by the need's containment-path count across selected edge usage journeys."""
