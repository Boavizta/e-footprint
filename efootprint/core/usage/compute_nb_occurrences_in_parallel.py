    ...
    for hour_shift in range(0, nb_of_full_hours_in_event_duration):
        if nb_avg_hourly_occurrences_in_parallel is None:
            ...
            nb_avg_hourly_occurrences_in_parallel_padded = np.pad(
                nb_avg_hourly_occurrences_in_parallel, (0, 1), constant_values=0)
            shifted_values = np.pad(nb_avg_hourly_occurrences_in_parallel, (1, 0), constant_values=0)
    ...
    if nonfull_duration_rest > 0:
        if nb_avg_hourly_occurrences_in_parallel is None:
            nb_avg_hourly_occurrences_in_parallel = (
                    hourly_occurrences_starts.value.astype(float) * nonfull_duration_rest)
        else:
            initial_values_padded = np.pad(
                nb_avg_hourly_occurrences_in_parallel, (0, 1), constant_values=0)
            shifted_values = np.pad(
                hourly_occurrences_starts.value.astype(float), (nb_of_full_hours_in_event_duration, 0),
                constant_values=0)
