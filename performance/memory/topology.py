...
class TopologyConfig:
    ...
    overlapping_pattern_subsets: bool = False
...
class SyntheticTopology:
    ...
    def recurrent_component_needs(self) -> tuple:
        return tuple(need for process in self.edge_processes for need in process.recurrent_edge_component_needs)


def _hourly_values(hours: int, volume: float):
    return create_source_hourly_values_from_list([volume] * hours, start_date=datetime(2026, 1, 5))
...
def _web_child(index: int) -> tuple[Storage, Server, Job, UsageJourneyStep]:
    ...
    job = Job.from_defaults(
        f"memory lab shared job{suffix}",
        server=server,
        data_transferred=SourceValue(2 * u.MB),
        data_stored=SourceValue(0.01 * u.MB_stored),
    )
    step = UsageJourneyStep(f"memory lab shared step{suffix}", user_time_spent=SourceValue(5 * u.min), jobs=[job])
...
def _edge_child(index: int, job: Job) -> tuple[EdgeFunction, RecurrentEdgeProcess, RecurrentServerNeed]:
    ...
    computer = EdgeComputer.from_defaults(
        f"memory lab edge computer{suffix}",
        storage=storage,
        compute=SourceValue(1000 * u.cpu_core),
        ram=SourceValue(1000 * u.GB_ram),
    )
    process = RecurrentEdgeProcess.from_defaults(f"memory lab shared edge process{suffix}", edge_device=computer)
...
def build_synthetic_topology(config: TopologyConfig = TopologyConfig()) -> SyntheticTopology:
    ...
    journey_pool_size = (
        config.journeys_per_pattern + config.pattern_count - 1
        if config.overlapping_pattern_subsets
        else config.journeys_per_pattern
    )
    web_children = (
        [_web_child(1)] * journey_pool_size
        if config.shared_children
        else [_web_child(index + 1) for index in range(journey_pool_size)]
    )
    web_journeys = tuple(
    ...
    for pattern_index in range(config.pattern_count):
        selection_start = pattern_index if config.overlapping_pattern_subsets else 0
        selected_journeys = web_journeys[selection_start : selection_start + config.journeys_per_pattern]
        weights = {journey: 1 + journey_index / 10 for journey_index, journey in enumerate(selected_journeys)}
        web_patterns.append(
            UsagePattern(
                f"memory lab web pattern {pattern_index + 1}",
                usage_journeys=weights,
    ...
    edge_children = (
        [_edge_child(1, jobs[0])] * journey_pool_size
        if config.shared_children
        else [_edge_child(index + 1, jobs[index]) for index in range(journey_pool_size)]
    )
    edge_journeys = tuple(
    ...
    for pattern_index in range(config.pattern_count):
        selection_start = pattern_index if config.overlapping_pattern_subsets else 0
        selected_journeys = edge_journeys[selection_start : selection_start + config.journeys_per_pattern]
        edge_patterns.append(
            EdgeUsagePattern(
                f"memory lab edge pattern {pattern_index + 1}",
                edge_usage_journeys=list(selected_journeys),
    ...
    system = System("memory lab system", usage_patterns=web_patterns, edge_usage_patterns=edge_patterns)
