...
### Changed
- Sankey construction now folds the cached attribution matrix as native kg floats, normalizes contextual objects and class visibility once, processes every requested lifecycle phase in one pass, and reuses those fold totals for component-breakdown normalization. Pint quantities are created only at the display boundary; the general attribution fold remains quantity-valued for programmatic callers.
...
- **Minimal persistence contract (schema v23), one canonical format.** A saved system now carries every object's inputs, the cached values of serialize-flagged slots — each impact source's `energy_footprint` + `instances_fabrication_footprint` pair, `System.total_footprint`, the `System.impact_repartition_matrix` computed structure and each edge device's new condensed `footprint_breakdown_summary` (period-summed per-component breakdown, what the Sankey decoration reads) — with their formulas and direct-ancestor addresses (children links derive by inversion at load), plus a values-free `calculation_graph` section: every computing slot's calculus and structural dependency edges, slot-addressed, with best-effort display labels. Anything absent recomputes on read, so the format is staleness-resilient by construction; saving peeks and never computes. `system_to_json` loses `save_calculated_attributes` and gains `save_computed_state=True` (`False` writes a pure inputs-only file for lean committed fixtures and from-scratch rebuilds); `ExplainableObject.to_json(save_calculated_attributes)` became `to_json(with_formula)`. Serialize-flagged computed structures are raw JSON-native values: list rows (the repartition matrix) round-trip as arrays and dict summaries (the edge-device breakdown) as objects. On the big performance fixture the saved file weighs 6.3% of the former exhaustive format (12.8 vs 202.8 MB) and loads 3.9x faster with zero computation.
- **Version-aware loading.** Stored computed values are trusted as slot caches only on an exact `efootprint_version` match (the calculation graph then reinstalls the dependency edges, so later edits — relationship changes included — invalidate precisely, without the former full-recompute fallback on the first relationship change). ANY mismatch (major, minor or patch) runs the schema upgrade handlers, then demotes surviving stored values to an in-memory, session-scoped "as computed by vX" baseline retained on each `System` — never re-serialized — and recomputes on read; `System.compare_to_version_baseline()` (with `System.has_version_baseline`) quantifies the upgrade drift through the standard `SystemComparison` machinery. The with-calculated-attributes detection flag, `json_to_system`'s `launch_system_computations` parameter and `from_json_dict`'s `is_loaded_from_system_with_calculated_attributes` parameter are gone: one format, one loader path, and loading never computes (objects hydrate under a private passive lifecycle state before an idempotent graph-wide transition makes the complete graph live, skipping `after_init`'s construction-time side effects).
- **Relationship mutation now has an explicit active/passive lifecycle.** Public list, dict and object mutations remain single `ModelingUpdate` transactions, while framework-owned construction, update apply/rollback, computed-facade write-back and serialization population use passive primitives that maintain ordering, reverse links and structural dependency nodes without recursively triggering another update. Augmented operators no longer issue duplicate transactions, and dict changes invalidate only the affected key when possible. The public modeling-update flag, activation methods and their propagation are removed; lifecycle transitions are framework-internal, so ordinary assignments and attached-container edits become transactional as soon as an object graph is live.
- **Updates eagerly validate but do not eagerly calculate outputs.** `ModelingUpdate` pulls invalidated guards and every guard on newly linked objects, while ordinary invalidated slots — including `System.total_footprint` — remain void until read. Callers that require immediate materialization may pass explicit `(object, attribute)` pairs through `eager_outputs`. System construction and object deletion likewise pull guards without warming the footprint. Computed-dict facades of objects a change detaches drop stale keys on the write path through `prune_stale_computed_dict_keys`.
- Impact attribution now uses the same caching paradigm as everything else: the former `@cached_property` attribution layer (server binding-resource shares, storage retention/baseline splits, edge-device pools, job attribution cells, step occupancy) became reactive computed slots — computed on read, cached in the reactive graph, and invalidated precisely through recorded dependency edges instead of being wiped wholesale after every update. The wipe machinery is gone: `flush_cached_properties_system_wide`, `flush_cached_properties`, `class_cached_property_names`, `render_cache` and the `flushed_memo` decorator; `functools.cached_property` is no longer used on modeling objects. Job occurrence and network-transfer primitives use per-coordinate computed-dict slots keyed by `(usage pattern, web step | recurrent server need)`, shared by attribution cells and network atoms.
- New `System.impact_repartition_matrix` computed structure: one dict-encoded row per attribution atom — (source, stream, cell coordinate ids, usage pattern, phase) with the value reduced to its period sum in kg — concatenating each source's `impact_repartition_rows` structure. `node_totals_and_links` (the Sankey feed) folds these stored summed scalars, so the fold behind every column/phase/exclusion combination touches no hourly data; `footprint_per_node[_per_source]` remains the live-atom batch read, while `attributed_footprint(obj, phase)` streams live atoms into only the requested object's total. Atoms and attributed outputs remain uncached.
- Assigning a computed attribute now raises an `AttributeError` in every context: computed values only enter their slot by computation or by the descriptor's explicit `attach_cached_value` (the JSON load path; tests pin with `tests.utils.patch_attribute` / `attach_attribute`). The legacy constructor pattern this rule replaces — subclasses forwarding dummy zeros to parent constructors for attributes they compute — is gone: the affected parent constructor params (`HardwareBase`/`Server(Base)` hardware specs, `JobBase` request needs, `EdgeComponent` per-unit specs, `EdgeDevice.structure_carbon_footprint_fabrication`, `RecurrentEdgeComponentNeed.recurrent_need`) default to `None` and are simply not stored when a subclass computes them. Trade-off: a forgotten required input on a concrete class now surfaces at first computation instead of as a constructor `TypeError`.
- Computation is now pull-based: the `@computed_attribute` / `@computed_dict` descriptors drive the reactive engine directly — reading a computed attribute computes and caches it in a per-instance `ReactiveSlot`, writes invalidate the changed nodes' dependency cones (deletion waves over calculus edges recorded from arithmetic ancestry plus structural edges recorded by relationship read hooks), and reverse relationships bump their nodes at every container-field transition. `ModelingUpdate` became apply → invalidate → guard validation → optional explicit output pulls, with rollback restoring inputs and revalidating. The push machinery is gone: `calculated_attributes` lists (now derived from the class computed-slot registry), the per-class dependent-objects declarations, the computation-chain builders and optimizer (with their framework→core back-edge), the synthesized `update_*` methods, and the update-time value snapshots. Canonical-class membership is now the derived `CANONICAL_CLASSES` registry (flattened `SANKEY_COLUMNS` + breakdown-only + non-Sankey classes) since pull semantics need no class ordering; Sankey column sorting keys off `SANKEY_COLUMNS` directly.
...
- The top-level `Sources` block holds only sources a serialized value actually references. Pure computed-attribute provenance (the EcoLogits `llm_impacts` / `compute_*_dag` function sources, the Boavizta cloud-instance API source) is re-attached deterministically whenever the value recomputes, so it is no longer hoisted — it was previously written as an orphan block entry no `source_id` pointed at. `system_to_json` gains `collect_referenced_source_ids` for the filter; the schema is unchanged and files still carrying the orphan entries load as before (extra unreferenced sources are harmless).

### Added
- Add context-local `observe_computations(callback)` scopes for monitoring successful reactive cache-miss completions without exposing modeled values. Observers are isolated across nested and concurrent contexts, restore cleanly after errors, and roll back a newly computed slot if their callback fails; a companion peek-only attribution-coverage query supports safe capacity guidance without triggering computation.
- `materialize_serialized_state(modeling_objects)`, an explicit export-boundary operation that computes every `serialize=True` slot before passive serialization builds a complete snapshot and its calculation graph. Ordinary saves remain peek-only and lazy.
- Computed descriptors accept a set of `ComputationPurpose` root tags. `FOOTPRINT` is declared only on meaningful system and physical-object footprint outputs; `computation_slots_for_purpose(instance, purpose)` derives intermediate membership from the materialized per-instance dependency graph, leaving unmaterialized or unreachable declarations unclassified.
- `@computed_attribute`, `@computed_dict` and `@computed_structure` accept a `serialize=True` flag — the single source of truth for what persists under the minimal contract (`serialized_slots(cls)`); computed structures expose the `peek` / `attach_cached_value` hooks serialization and loading use, and `computed_dict` facades only serialize when fully cached.
- Schema migration handler 22 to 23: drops legacy stored computed entries, so pre-contract files upgrade to inputs-only and recompute on read.
- Perf gates in `tests/performance_tests/test_perf_gates.py`, diffed against the committed pre-overhaul baselines: serialized weight at most 30% of the exhaustive format (measured 6.3%), load-to-ready at least 3x faster (3.9x), 1,000-iteration nudge-and-read loop at least 10x faster (22.4x on the finest calibration target), engine bookkeeping under 5% of recompute wall time (2.2%), peak build memory within the eager baseline (575 vs 590 MB).
- Parity-harness variant starting from a freshly deserialized canonical file (cached footprints below valueless intermediates), plus round-trip identity, version-trust, migration and on-demand auditability tests (`tests/api_utils_tests/test_minimal_serialization_contract.py`).
- Reactive pull-engine primitives in `efootprint/abstract_modeling_classes/reactive_core.py`: `ReactiveSlot` (cached/void slot state plus a one-bit invalidation-wave marker), the exception-safe contextvar compute stack (doubles as cycle guard, reporting circular dependencies with the offending slot chain), calculus + structural dependency-edge recording with edges refreshed on every successful recompute (a failed one keeps the previous edges), and the `invalidate` deletion wave (deletes cached values along the edges; topology edges and the marker survive valuelessness, so a wave crosses valueless intermediates to reach cached descendants — the state a partially loaded file produces). The engine is not yet wired to the `@computed_attribute` descriptors: production behavior is unchanged (still the eager machinery); the engine is covered by synthetic-graph unit tests.
...
### Added
- `ModelingObject.weight_labels` class metadata: the static label of each weighted dict relationship attribute ("Times per journey" / "Times per step" / "Times per occurrence" / "Count in group"), declared on `UsageJourney`, `UsageJourneyStep`, `RecurrentServerNeed` and `EdgeDeviceGroup` and used by their constructors. Single source of truth for external consumers (e.g. the web interface) that build weight entries themselves.
- `to_weighted_explainable_object_dict` normalizer in `explainable_object_dict.py`: turns constructor sugar (a list of keys with duplicates accumulating, or a dict with plain-number values) into a `WeightedExplainableObjectDict` of dimensionless, non-negative `SourceValue` weights — a small `ExplainableObjectDict` subclass that enforces the weight invariant on every `__setitem__`, so mutations are validated like construction. The subclass survives JSON round-trips (dict attributes are rebuilt with the class declared by their `__init__` annotation) and `ModelingUpdate` replacements. `EdgeDeviceGroup` adopts it, so `EdgeDeviceGroup(name, sub_group_counts=[g1], edge_device_counts={d1: 3})` now works, with the count invariant enforced solely on set (no eager validator). `ExplainableObjectDict.__setitem__` stays strict; no serialized output changes.
...
### Changed
- `ImpactRepartitionSankey` rewritten as a column-walk over the attribution fold; constructor surface and presentation semantics (phase columns, breakdown decorations, small-node aggregation, skip/exclude options) are unchanged. System-level totals are identical on all fixture models; per-node values change only where the revamp deliberately fixes attribution: dual-side jobs now split their server impact between web steps and edge recurrent server needs by occurrence shares, edge component needs re-split by the equal-share idle floor and held-volume EdgeStorage demand, plus small server-side shifts from the binding-resource / provisioned-share weights.
- Performance: cold big-system attribution fold drops ~1.26 s → ~0.35 s (plain-float accumulation, period-total scalars instead of hourly arrays in the Sankey fold, ancestor-dedup fix in `ExplainableObject.__init__`, memoized JobBase occurrence primitives) and the unit registry caches `u.<unit>` attribute lookups (cold Sankey on a real edge-heavy model ~436 → ~332 ms).
...
### Removed
- All legacy attribution machinery (net ~700 production lines): `ModelingObject`'s `*_impact_repartition*` calculated attributes, `attributed_*_footprint_per_source[_resolved]` and resolution helpers, and every per-source repartition weight (ServerBase job weights, Storage shared-storage weights, Network overrides, Device per-step weights, EdgeDevice component-need weight chaining, external-API pass-throughs, container neutral-remainder logic).
...
### Fixed
- System servers to also take into account servers linked to edge usage patterns.

### Changed
- Updated to BoaviztAPI version 2.
...
### Fixed
- Add custom self_delete method to EdgeComputer and EdgeAppliance objects, because their hidden components need to be deleted when the EdgeComputer or EdgeAppliance is deleted.
...
### Changed
- Removed single edge usage pattern to edge usage journey constraint. Now several edge usage pattern can have the same edge usage journey as parameter.
- Removed single edge usage journey to edge process link constraint. Now edge usage journeys can be reused in different edge processes.
- Renamed SourceRecurringValues and ExplainableRecurringQuantities into SourceRecurrentValues and ExplainableRecurrentQuantities, to keep consistency with RecurrentEdgeProcess.
