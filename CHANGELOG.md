...
- `EdgeUsagePattern.edge_usage_journeys` combines several functionality bundles on one physical deployment. Deployment starts and lifetime now belong to the pattern; distinct bundle needs aggregate on shared hardware while the chassis, components, and fixed footprint are counted once.
...
### Fixed
- Web journey relationships reject non-finite weights, and aggregate edge results no longer depend on functionality-bundle order.
- V24 migration marks renamed web occurrence and edge deployment-start series with the correct `occurrence` unit, keeping migrated models valid through round trips and interface validation.
...
- Edge devices without components no longer share reactive values during legacy model imports.
- Tutorial generation now captures and embeds Matplotlib plots reliably.
...
- Reverse relationships and relationship-container mutations now precisely invalidate dependent values and roll back automatically when validation fails.
- Serialized `Sources` now contains only referenced sources; computed provenance is restored when values recompute.
- The architecture documentation is now a task-oriented HTML hub under `specs/architecture/`.

### Added
- `@computed_attribute`, `@computed_dict`, and `@computed_structure`, including opt-in serialization and computation-purpose tags.
- `observe_computations(callback)` for monitoring successful cache-miss computations.
- `materialize_serialized_state(modeling_objects)` for explicitly computing a complete export snapshot.
- Performance, parity, migration, and randomized-mutation regression gates for the reactive engine and persistence format.

### Removed
- Simulation machinery, including `ModelingUpdate.simulation_date`; use `System.compare_to` for what-if analysis.
- Previous/initial footprint capture and `System.plot_emission_diffs`; use duplicated systems and `SystemComparison` instead.
- Legacy eager-computation machinery and cache-flush APIs.

### Breaking API changes
- `system_to_json(save_calculated_attributes=...)` becomes `system_to_json(save_computed_state=...)`.
- `ExplainableObject.to_json(save_calculated_attributes)` becomes `to_json(with_formula)`.
- `json_to_system(..., launch_system_computations=...)` and `from_json_dict(..., is_loaded_from_system_with_calculated_attributes=...)` are removed.
