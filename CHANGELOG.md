...
### Changed
- **Minimal persistence contract (schema v23), one canonical format.** A saved system now carries every object's inputs, the cached values of serialize-flagged slots — each impact source's `energy_footprint` + `instances_fabrication_footprint` pair, `System.total_footprint`, the lazy `System.impact_repartition_matrix` and each edge device's new condensed `footprint_breakdown_summary` (period-summed per-component breakdown, what the Sankey decoration reads) — with their formulas and direct-ancestor addresses (children links derive by inversion at load), plus a values-free `calculation_graph` section: every computing slot's calculus and structural dependency edges, slot-addressed, with best-effort display labels. Anything absent recomputes on read, so the format is staleness-resilient by construction; saving peeks and never computes. `system_to_json` loses `save_calculated_attributes` and gains `save_computed_state=True` (`False` writes a pure inputs-only file for lean committed fixtures and from-scratch rebuilds); `ExplainableObject.to_json(save_calculated_attributes)` became `to_json(with_formula)`. Serialize-flagged lazy values are raw JSON-native structures: list rows (the repartition matrix) round-trip as arrays and dict summaries (the edge-device breakdown) as objects. On the big performance fixture the saved file weighs 6.3% of the former exhaustive format (12.8 vs 202.8 MB) and loads 3.9x faster with zero computation.
- **Version-aware loading.** Stored computed values are trusted as slot caches only on an exact `efootprint_version` match (the calculation graph then reinstalls the dependency edges, so later edits — relationship changes included — invalidate precisely, without the former full-recompute fallback on the first relationship change). ANY mismatch (major, minor or patch) runs the schema upgrade handlers, then demotes surviving stored values to an in-memory, session-scoped "as computed by vX" baseline retained on each `System` — never re-serialized — and recomputes on read; `System.compare_to_version_baseline()` (with `System.has_version_baseline`) quantifies the upgrade drift through the standard `SystemComparison` machinery. The with-calculated-attributes detection flag, `json_to_system`'s `launch_system_computations` parameter and `from_json_dict`'s `is_loaded_from_system_with_calculated_attributes` parameter are gone: one format, one loader path, and loading never computes (the loader enables live updates through the new `ModelingObject.enable_modeling_updates`, skipping `after_init`'s construction-time side effects).
- **The eager set narrows to the system total footprint.** `ModelingUpdate` gains a per-call `eager_outputs` parameter: by default the affected system's `total_footprint` recomputes at update time (errors in the change's footprint cone keep surfacing immediately), tight loops pass `eager_outputs=()` and read just the value they need — 1,000 iterations of "nudge one input, read one downstream target" run 22x faster than the eager engine's per-update baseline on the big fixture. Slots whose getters enforce user-facing constraints outside the footprint cone are declared `guard=True` (server and edge available-capacity checks, edge-storage cumulative-capacity check; `<name>_validation` slots are guards implicitly) and always recompute when an update invalidates them, so invalid edits are still rejected at update time. Linking a *new* object into a live system also computes every guard slot of the object and its linked subtree at update time (a never-computed slot has no dependency edges, so no invalidation wave could reach it), so an invalid new link — e.g. an edge device whose lifespan is shorter than its usage journey's span — is rejected and rolled back like any invalid edit, whatever the eager set. Computed-dict facades of objects a change detaches now drop their stale keys on the write path (`prune_stale_computed_dict_keys`) instead of relying on the retired recompute-everything sweep.
- Impact attribution now uses the same caching paradigm as everything else: the former `@cached_property` attribution layer (server binding-resource shares, storage retention/baseline splits, edge-device pools, job attribution cells, step occupancy) became `@lazy_attribute` projection slots — computed on read, cached in the reactive graph, and invalidated precisely through recorded dependency edges (a one-input edit voids only the attribution slots in its cone) instead of being wiped wholesale after every update. The wipe machinery is gone: `flush_cached_properties_system_wide`, `flush_cached_properties`, `class_cached_property_names`, `render_cache` and the `flushed_memo` decorator; `functools.cached_property` is no longer used on modeling objects. Job's per-(pattern, step/need) occurrence and data-transfer memos are now plain methods.
- New `System.impact_repartition_matrix` lazy slot: one dict-encoded row per attribution atom — (source, stream, cell coordinate ids, usage pattern, phase) with the value reduced to its period sum in kg — concatenating each source's `impact_repartition_rows` (a lazy slot on the new `AttributionSource` mixin). `node_totals_and_links` (the Sankey feed) now folds these stored summed scalars, so the fold behind every column/phase/exclusion combination touches no hourly data (the Sankey's breakdown-by-source decoration reads each edge device's serialize-flagged `footprint_breakdown_summary`); `footprint_per_node[_per_source]` and `attributed_footprint` remain hourly reads folding live atoms on every call (no memoization).
...
### Added
- `@computed_attribute`, `@computed_dict` and `@lazy_attribute` accept a `serialize=True` flag — the single source of truth for what persists under the minimal contract (`serialized_slots(cls)`); `lazy_attribute` gained the `peek` / `attach_cached_value` hooks serialization and loading use, and `computed_dict` facades only serialize when fully cached.
...
- Parity-harness variant starting from a freshly deserialized canonical file (cached footprints below valueless intermediates), plus round-trip identity, version-trust, migration and lazy-auditability tests (`tests/api_utils_tests/test_minimal_serialization_contract.py`).
