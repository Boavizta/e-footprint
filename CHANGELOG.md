...
- **Updates eagerly validate but do not eagerly calculate outputs.** `ModelingUpdate` pulls invalidated guards and every guard on newly linked objects, while ordinary invalidated slots — including `System.total_footprint` — remain void until read. Callers that require immediate materialization may pass explicit `(object, attribute)` pairs through `eager_outputs`. System construction likewise pulls guards across the linked model without warming the footprint. Computed-dict facades of objects a change detaches drop stale keys on the write path through `prune_stale_computed_dict_keys`.
