...
- `Dockerfile`: a minimal `python:3.12-slim` image that installs `efootprint` from PyPI, for running a model with no local Python setup (`docker run --rm -v "$PWD":/work boavizta/efootprint python /work/build_model.py`). Documented in the README's new "Run with Docker" section. `.github/workflows/docker-publish.yml` builds, smoke-tests, and publishes it to Docker Hub as `boavizta/efootprint` on each GitHub release; the publish step is documented in `RELEASE_PROCESS.md`. Requires `DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` repo secrets to be provisioned before the first release-triggered push.
