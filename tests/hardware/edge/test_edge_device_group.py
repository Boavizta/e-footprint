...
class TestEdgeDeviceGroupInit(TestCase):
    ...
    def test_calculated_attributes_includes_counts_validation(self):
        group = make_group("G")
        self.assertIn("counts_validation", group.calculated_attributes)

    def test_calculated_attributes_includes_effective_nb(self):
        group = make_group("G")
        self.assertIn("effective_nb_of_units_within_root", group.calculated_attributes)
    ...
    def test_modeling_objects_whose_attributes_depend_on_me_with_sub_group(self):
        parent = make_group("Parent")
        child = make_group("Child")
        parent.sub_group_counts[child] = SourceValue(2 * u.dimensionless)
        result = parent.modeling_objects_whose_attributes_depend_directly_on_me
        self.assertIn(child, result)

    def test_modeling_objects_whose_attributes_depend_on_me_with_device(self):
        group = make_group("Group")
        mock_device = create_mod_obj_mock(EdgeDevice, "Device 1")
        group.edge_device_counts[mock_device] = SourceValue(4 * u.dimensionless)
...
class TestEdgeDeviceGroupFindParentGroups(TestCase):
    ...
    def test_parent_is_not_duplicated(self):
        parent = make_group("Parent")
        child = make_group("Child")
        parent.sub_group_counts[child] = SourceValue(2 * u.dimensionless)
        result = child._find_parent_groups()
    ...
    def test_ignores_non_group_containers(self):
        """A group used as a key in a non-group dict should not show up as parent."""
        group = make_group("Group")
        # No EdgeDeviceGroup parents
        self.assertEqual([], group._find_parent_groups())
