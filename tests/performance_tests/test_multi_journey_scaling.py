...
class TestMultiJourneyScaling(TestCase):
    def test_shared_topology_reuses_children_without_reselecting_journeys(self):
        ...
        topology = build_synthetic_topology(TopologyConfig(
            modeled_hours=24, pattern_count=2, journeys_per_pattern=4, shared_children=True))
    ...
    def test_hourly_caches_follow_paths_and_distinct_pattern_need_pairs(self):
        ...
        topology = build_synthetic_topology(TopologyConfig(
            modeled_hours=24, pattern_count=pattern_count,
            journeys_per_pattern=journeys_per_pattern, shared_children=True))
        ...
        self.assertEqual(expected_web_paths, cached_sub_slot_count(
        ...
        self.assertEqual(
            ...
            cached_sub_slot_count(system, "unitary_hourly_need_per_usage_pattern"))
        self.assertEqual(
            expected_server_pairs,
            cached_sub_slot_count(system, "unitary_hourly_volume_per_usage_pattern"))

        component_path_count = sum(
            len(pattern.containment_inventory.component_need_paths)
            for pattern in system.edge_usage_patterns)
        server_path_count = sum(
            len(pattern.containment_inventory.server_need_paths)
            for pattern in system.edge_usage_patterns)
    ...
    def test_scalar_rows_grow_with_paths_without_retaining_source_helpers(self):
        ...
        one_journey = build_synthetic_topology(TopologyConfig(
            modeled_hours=24, pattern_count=2, journeys_per_pattern=1, shared_children=True)).system
        four_journeys = build_synthetic_topology(TopologyConfig(
            modeled_hours=24, pattern_count=2, journeys_per_pattern=4, shared_children=True)).system
        ...
        self.assertEqual(
            4 * one_state["attribution_matrix_rows"], four_state["attribution_matrix_rows"])
