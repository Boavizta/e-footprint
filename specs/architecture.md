...
- **Minimal persistence contract (since v23).** One canonical format: every object's inputs, the cached values of serialize-flagged slots (each impact source's `energy_footprint` + `instances_fabrication_footprint` pair, `System.total_footprint`, the lazy `System.impact_repartition_matrix` and `EdgeDevice.footprint_breakdown_summary`) with their formulas and ancestor addresses (children derived by inversion at load), and a values-free `calculation_graph` section (every computing slot's calculus + structural dependency edges, slot-addressed, plus best-effort labels). Anything absent is valueless and recomputes on read — staleness-resilient by construction. Saving peeks, never pulls (a save after an edit simply omits the flagged slots the edit voided). `system_to_json(system, save_computed_state=False)` writes a pure inputs-only file (lean committed fixtures, from-scratch rebuilds).
- **Version-aware loading.** On an exact `efootprint_version` match, stored values attach as trusted slot caches and the calculation graph reinstalls the dependency edges (zero computation; later edits invalidate through the reinstalled graph, structural edges included). On ANY mismatch, major-version schema handlers run, then surviving stored values demote to an in-memory, session-scoped "as computed by vX" baseline retained on each System — never re-serialized; `System.compare_to_version_baseline()` quantifies the upgrade drift with the standard comparison machinery. The schema version is the major library version; the 22→23 handler drops legacy stored computed entries.
- **`json_to_system.py`** / **`system_to_json.py`** — serialization round-trip. **Reference resolution heuristic:** in `ModelingObject.from_json_dict`, a scalar string attribute is interpreted as an object reference when it equals an already-built object's `id`. `id` and `name` are exempt (names are plain labels, never references) — otherwise an object whose `name` equals another object's `id` (e.g. a second "France" country alongside the catalog one keyed `"France"`) would have its name silently resolved into that object.
...
Computation is pull-based: reading a computed attribute computes and caches it on demand. The `__setattr__` override in `ModelingObject` routes live input writes through `ModelingUpdate`, which invalidates the changed slots' dependency cones and eagerly recomputes the configured `eager_outputs` — by default the affected system's total footprint (so computation errors in the change's footprint cone surface at update time); tight loops pass `eager_outputs=()` and let values compute on the next read. Guard slots (`<name>_validation` names, or any slot declared with `guard=True` because its getter enforces a user-facing constraint outside the footprint cone, e.g. edge capacity checks) always recompute when invalidated, whatever the eager set; and since a never-computed slot has no dependency edges for a wave to reach, `ModelingUpdate` additionally computes every guard slot of the objects a change newly links into the model (their linked subtree included), so invalid new links are rejected at update time too.
...
`ModelingUpdate` (they stay void until the next render). The matrix and each edge device's
`footprint_breakdown_summary` (period-summed per-component breakdown, the Sankey decoration's read) are
declared `@lazy_attribute(serialize=True)`: they persist when materialized — filled on first Sankey use —
so a loaded session renders every Sankey combination from stored data.
