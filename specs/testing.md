...
- **Performance tests** (in `tests/performance_tests/`) — large-system benchmarks. The big-system fixtures (`big_system.json`, `big_system_with_calc_attr.json`) are gitignored (too heavy to commit) and regenerated locally with `python tests/performance_tests/generate_big_system.py`; the fixture-integrity tests skip when they are absent (e.g. in CI). `baseline_results.json` is the committed benchmark reference file (force-added past the repo-wide `*.json` ignore), regenerated by `benchmark_baselines.py` and diffed against when evaluating engine changes.
