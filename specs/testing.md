...
- **Unit tests** (in `tests/`) — individual `ModelingObject` classes in isolation. Focus: computed-attribute getters, validation logic, error handling.
...
- `default_values` or `calculated_attributes` contents.
...
1. **Computed-attribute getters** — happy path, error conditions, boundary cases. Read the attribute to compute it; use `recompute_attribute(obj, "attr"[, key])` from `tests/utils.py` to force a fresh computation after changing raw inputs in place, and `patch_attribute(obj, "attr", value)` to pin a computed (or input) attribute for a with-block without computing it.
2. **Validation logic.**
...
requires every computed getter containing a direct `raise` or `assert` to use `guard=True`. It deliberately
detects only explicit failures in the getter body; Python exceptions raised transitively by arithmetic,
conversions, helpers, or libraries are not statically enumerable.
...
class TestClassName(TestCase):
    ...
    def test_attribute_happy_path(self):
        """Test attribute computation in normal conditions."""
    ...
    def test_attribute_invalid_input(self):
        """Test attribute computation raises error for invalid input."""
        self.mock_dependency.value = -1

        with self.assertRaises(ValueError) as context:
            recompute_attribute(self.test_object, "attribute")
...
When testing computed attributes, set all input values **in the test itself** (or add a comment with the formula), so developers don't have to navigate back to setUp:
...
def test_total_energy(self):
    ...
    self.assertEqual(expected, recompute_attribute(self.test_object, "total_energy"))
```
...
`total_footprint` only walks the impact-source side of the graph; the attribution layer (atom builders, folds, `attribution.attributed_footprint`, Sankey-facing paths, the repartition-matrix slots) lives in `@lazy_attribute` projection slots and plain methods that fire only when something asks for them. Bugs in those paths stay invisible until a UI feature reaches in.

`IntegrationTestBaseClass.run_test_materialize_all_lazy_projections` runs on every fixture (code + `_from_json`) and forces every `@lazy_attribute` slot on every linked object (and the system, including `impact_repartition_matrix`) to compute. A successfully-built system should always materialize cleanly. When adding a new lazy projection to a modeling class, this test will cover it automatically — no per-feature assertion needed. Unit tests read lazy attributes like any attribute; `recompute_attribute` / `attach_attribute` / `patch_attribute` from `tests/utils.py` handle them exactly like computed slots.
