...
## Defending against on-demand attribution bugs

`total_footprint` only walks the impact-source side of the graph; the attribution layer (atom builders, folds, `attribution.attributed_footprint`, Sankey-facing paths, the repartition-matrix structures) lives in computed slots and plain methods that fire only when something asks for them. Bugs in those paths stay invisible until a UI feature reaches in.

`IntegrationTestBaseClass.run_test_materialize_all_computed_structures` runs on every fixture (code + `_from_json`) and forces every `@computed_structure` on every linked object (and the system, including `impact_repartition_matrix`) to compute. A successfully built system should always materialize cleanly. When adding a computed structure to a modeling class, this test covers it automatically. Unit tests read structures like any attribute; `recompute_attribute` / `attach_attribute` / `patch_attribute` from `tests/utils.py` handle them exactly like other computed slots.
