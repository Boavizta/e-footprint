...
- Describe the current architecture, not the sequence of designs that preceded it. Do not retain legacy-oriented explanations merely to justify the present shape. Preserve historical rationale only when it is a genuine Chesterton's fence: changing an apparently unnecessary constraint would violate a still-current invariant. In that case, state the invariant and concrete failure mode rather than narrating obsolete machinery.
