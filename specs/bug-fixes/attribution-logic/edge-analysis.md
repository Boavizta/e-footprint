...
- **Fabrication** (neutral): `(comp_fab[C,up] + chassis_fab[up] / nb_components) × s_dem(need, up) + pool_share(up)` — the chassis rides with each component as an equal `1/nb_components` share (matching the breakdown axis), and every carrier at the pattern adds its equal share of the unused-components pool (below). Σ over all carriers = `chassis_fab[up] + Σ_C comp_fab[C,up] = instances_fabrication_footprint_per_usage_pattern[up]`.

**Unused components are part of the chassis.** A component with no needs at a pattern the device serves books `T × N[up] × (embodied carbon / lifespan) × 1h` eagerly with the deployment, exactly like the structure (`EdgeDevice.unused_component_fabrication_per_edge_device`, computed from the component's *input* attributes because need-less components never enter the calculated-attribute computation chain) — so `instances_fabrication_footprint_per_usage_pattern[up]` always carries the whole device. At attribution, the pool `Σ over unused C of (comp_fab_from_inputs[C,up] + chassis_fab[up]/nb_components)` (the full structure when the device has no components) splits **equally across the pattern's deployment carriers**: the component needs at up and the device's `RecurrentServerNeed`s reached at up. A need's pool share rides inside its fabrication atom value; an RSN's share is emitted as `(rsn, ef)` atoms split by the same occurrence ratios as pairs 8/12. A pattern reached only through RSNs therefore sends the whole device fabrication through them; a pattern that books fabrication with **no** carriers (an empty `RecurrentEdgeDeviceNeed`) raises. Energy needs no such rule: it is need-booked on both the eager and attribution sides, so unused components and RSN-only patterns book none.
...
## EdgeDevice -> EdgeComponent (breakdown by source)

`fabrication_footprint_breakdown_by_source[C] = T × (C.fabrication_footprint_per_edge_device + Σ over patterns where C is unused of unused_component_fabrication_per_edge_device(C, up)) + chassis_total / nb_components`; `energy_footprint_breakdown_by_source[C] = T × C.energy_footprint_per_edge_device`. The orthogonal hardware axis, the single place the chassis is split (equally); each component also shows its own deployment-booked fabrication at the patterns where it is unused, so the breakdown always sums to the device total. The atom fold by component matches the breakdown exactly when every component has needs at every pattern the device serves; where it doesn't, the unused component's fabrication and chassis share are carried by the pattern's other carriers (a who-pays axis), while the breakdown keeps them on the component itself (a what-hardware axis).
...
- **Fixes**: (1) zero-demand fallback → equal share (was `fallback=0`, dropping footprint at idle hours); (2) EdgeStorage demand → cumulative held volume (was the net write rate, negative on deletes); (3) idle/base energy split off and shared equally (the single demand weight mis-split it across needs at partial-demand hours — worst on mostly-idle devices, where idle dominates the energy); (4) unused components are part of the chassis — their embodied carbon is deployment-booked in the eager per-pattern totals (an **eager change**: previously a need-less component's fabrication was booked nowhere) and attributed by an equal split across the pattern's carriers (component needs + RecurrentServerNeeds), closing the conservation gap for need-less components and RSN-only patterns that the legacy weights papered over by normalization (smearing the chassis onto the wrong needs, patterns and countries).
